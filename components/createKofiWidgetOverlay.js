const kofiWidgetOverlayConfig = {
  ...
  'floating-chat.core.closer':
    '<svg height="0px" width="15px"><line x1="2" y1="8" x2="13" y2="18" style="stroke:#000; stroke-width:3" /><line x1="13" y1="8" x2="2" y2="18" style="stroke:#000; stroke-width:3" /></svg>',
  'floating-chat.core.position.bottom-left':
    'position: fixed; bottom: 50px; left: 10px; width: 160px; height: 65px;',
  'floating-chat.cssId': '',
...
var kofiWidgetOverlayFloatingChatBuilder =
  ...
  function (config, _utils) {
    ...
    var write = function (parentElementId) {
      ...
      var iframeContainerElement = createButtonContainerIframe(
        iframeId,
        'https://storage.ko-fi.com/cdn/scripts/floating-chat-main.css'
      )
      var mobiIframeContainerElement = createButtonContainerIframe(
        mobiIframeId,
        'https://storage.ko-fi.com/cdn/scripts/floating-chat-main.css'
      )
      _utils.loadStyleSheet(
      ...
      if ('' !== styleSheetsValue) {
        const styleSheets = JSON.parse(styleSheetsValue)
      ...
      var desktopDonateButton = attachDonateButton(
        iframeContainerElement,
        iframeId,
        {popupId: 'kofi-popup-iframe', popupIframeContainerIdSuffix: 'popup-iframe-container'},
        {maxHeight: 690, minHeight: 400}
      )
      widgetPageLoadInitiatedStates.push([desktopDonateButton, false])
      var mobileDonateButton = attachDonateButton(
        mobiIframeContainerElement,
        mobiIframeId,
        {
    ...
    function slidePopupOpen(popup, finalHeight) {
      ...
      document.getElementsByClassName('floating-chat-kofi-popup-iframe-notice')[0].style.display =
        'block'
    }
    function closePopup(popup, donateButton) {
      popup.style =
        'height: 0px; width:0px; transition:height 0.3s ease 0s , width 1s linear,opacity 0.3s linear; opacity:0;'
    ...
    function insertPopupHtmlIntoBody(donateButton, selectors, parentElementId) {
      ...
      const linkText = document.createTextNode(noticeText)
    ...
    function toggleKofiIframe(iframeId, state, donateButton, selectors, heightLimits) {
      ...
      if (state === 'open') {
        ...
        var finalHeight =
          window.innerHeight - (window.innerHeight - iframeContainerParent.offsetTop) - 60
    ...
    var getHtml = function () {
      ...
      var donateButtonBackgroundColor = _configManager.getValue(
        _myType,
        'donateButton.background-color'
      )
    ...
    const loadKofiIframe = function (pageId, parentElementId, iframeStyle) {
      ...
      const tryLoad = function () {
        if (!_iframeLoading) {
          _iframeLoading = true
          let url =
            kofiWidgetOverlayConstants.kofiRoot + pageId + '/?hidefeed=true&widget=true&embed=true'
          if (_showFeed) {
    ...
    const mergeOptions = function (optionSetA, optionSetB) {
      for (var property in optionSetA) {
        if (optionSetA.hasOwnProperty(property)) {
          optionSetA[property] =
            optionSetB[property] !== undefined ? optionSetB[property] : optionSetA[property]
        }
      }
    }
...
export default function createKofiWidgetOverlay() {
  ...
  const setConfigDefaults = function (config, widgetType, pId, instanceId) {
    ...
    config[widgetType + '.cssId'] =
      config[widgetType + '.cssId'] !== undefined && config[widgetType + '.cssId'] !== ''
        ? config[widgetType + '.cssId']
        : instanceId
    config[widgetType + '.stylesheets'] =
      config[widgetType + '.stylesheets'] !== undefined
        ? config[widgetType + '.stylesheets']
        : '["https://fonts.googleapis.com/css?family=Nunito:400,700,800&display=swap"]'
    return config
  }
    ...
    if (isFirstRender) {
      ...
      if (containerId != null) {
        document.getElementById(
          containerId
        ).innerHTML += `<div id="${parentButtonWrapperId}"></div>`
      } else {
